...
describe('AiBotClassifier', () => {
  ...
  describe('classify_user_agent', () => {
    ...
    it('should classify GPTBot user agent', () => {
      const result = classify('Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatible; GPTBot/1.2; +https://openai.com/gptbot)');
    ...
    it('should classify ChatGPT-User agent', () => {
      const result = classify('Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatible; ChatGPT-User/1.0; +https://openai.com/bot)');
      expect(result.$is_ai_bot).toBe(true);
    ...
    it('should classify OAI-SearchBot agent', () => {
      const result = classify('Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatible; OAI-SearchBot/1.0; +https://openai.com/searchbot)');
      expect(result.$is_ai_bot).toBe(true);
    ...
    it('should classify ClaudeBot agent', () => {
      const result = classify('Mozilla/5.0 (compatible; ClaudeBot/1.0; +claudebot@anthropic.com)');
      expect(result.$is_ai_bot).toBe(true);
    ...
    it('should classify Applebot-Extended agent', () => {
      const result = classify('Mozilla/5.0 (Macintosh; Intel Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Applebot-Extended/0.1');
      expect(result.$is_ai_bot).toBe(true);
    ...
    it('should classify Meta-ExternalAgent agent', () => {
      const result = classify('Mozilla/5.0 (compatible; Meta-ExternalAgent/1.0)');
      expect(result.$is_ai_bot).toBe(true);
    ...
    it('should NOT classify regular Chrome browser as AI bot', () => {
      const result = classify('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36');
    ...
    it('should NOT classify Googlebot (regular) as AI bot', () => {
      const result = classify('Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)');
      expect(result.$is_ai_bot).toBe(false);
    });

    it('should NOT classify Bingbot (regular) as AI bot', () => {
      const result = classify('Mozilla/5.0 (compatible; bingbot/2.0; +http://www.bing.com/bingbot.htm)');
      expect(result.$is_ai_bot).toBe(false);
    });
    ...
    it('should return all expected fields for a match', () => {
      ...
      expect(['indexing', 'retrieval', 'agent']).toContain(result.$ai_bot_category);
    });
