...
var create_client = function(token, config) {
    ...
    */
    ...
    var MP_BATCH_SIZE = 50; // Mixpanel API permits 50 events per request
    metrics.import_batch = function(event_list, options, callback) {
        var data_length = event_list.length,
            total_requests = Math.ceil(data_length / MP_BATCH_SIZE),
            max_simultaneous_requests = total_requests,
        ...
        if (options.max_batch_size) {
            max_simultaneous_requests = Math.min(
                total_requests, Math.max(Math.floor(options.max_batch_size / MP_BATCH_SIZE), 1)
            );
        }

        var send_batch = function() {
            event_batch = [];
            for (var ei = event_group_idx; ei < data_length && ei < event_group_idx + MP_BATCH_SIZE; ei++) {
                properties = event_list[ei].properties;
                properties.time = parse_time(properties.time);
                if (!properties.token) {
                    properties.token = metrics.token;
                }
                if (!properties.mp_lib) {
                    properties.mp_lib = 'node';
                }
            ...
            if (event_batch.length > 0) {
                ...
                event_group_idx += MP_BATCH_SIZE;
            }
        };

        if (metrics.config.debug) {
            console.log("Sending " + event_list.length + " events to Mixpanel in " + total_requests + " requests");
        }

        for (var ri = 0; ri < max_simultaneous_requests; ri++) {
            send_batch();
        }
    };
