...
var create_client = function(token, config) {
     ...
     * @param {object}   options
     * @param {Array}    options.list                       array of objects
     * @param {string}   options.endpoint                   e.g. `/track` or `/import`
     * @param {Function} options.prepare_item_fn            function which called on all elements
    ...
    var send_batch_requests = function(options, callback) {
        var list = options.list,
            endpoint = options.endpoint,
            prepare_item_fn = options.prepare_item_fn,
            ...
            batches = chunk(list, max_batch_size),
            request_batches = max_concurrent_requests ? chunk(batches, max_concurrent_requests) : [batches],
         ...
         * @param {Function} cb
         */
        function send_batch(batch, cb) {
            if (batch.length > 0) {
                batch = batch.map(prepare_item_fn);
                // must be a POST
        ...
        function send_next_request_batch(index) {
            ...
            async_all(request_batch, send_batch, cb);
        }
        ...
        if (metrics.config.debug) {
            console.log(
                "Sending " + list.length + " item(s) to Mixpanel in " +
                total_batches + " batches of items and " +
    ...
    metrics.track_batch = function(event_list, options, callback) {
        ...
        var batch_options = {
            ...
            prepare_item_fn: prepare_event_for_send
        };

        send_batch_requests(batch_options, callback);

        function prepare_event_for_send(event) {
    ...
    */
    ...
    metrics.people = {
        ...
        */
        ...
        /**
         * send a batch of people updates to mixpanel `engage` endpoint
         * @param {Array}    changes                            array of people changes objects to track
         * @param {object}   [options]
         * @param {number}   [options.max_concurrent_requests]  number of concurrent http requests that can be made to mixpanel
         * @param {number}   [options.max_batch_size]           number of events that can be sent to mixpanel per request
         * @param {Function} [callback]                         callback receives array of errors if any
         */
        batch: function (changes, options, callback) {
            var batch_options = {
                list: changes,
                endpoint: "/engage",
                max_concurrent_requests: options.max_concurrent_requests,
                max_batch_size: options.max_batch_size,
                prepare_item_fn: prepare_item_for_send
            };

            send_batch_requests(batch_options, callback);

            function prepare_item_for_send(item) {
                if (item.$time) {
                    item.$time = ensure_timestamp(item.$time);
                }
                item.$token = metrics.token;
                return item;
            }
        }
    };
