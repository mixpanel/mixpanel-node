        ...
        */
        remove: function(distinct_id, data, modifiers, callback) {
            ...
            if (typeof(data) !== 'object' || util.isArray(data)) {
                if (metrics.config.debug) {
                    console.error("Invalid value passed to mixpanel.people.remove - data must be an object with" +
                      " scalar values");
                }
                return;
            }
            ...
            Object.keys(data).forEach(function(key) {
                ...
                if (typeof(val) === 'string' || typeof(val) === 'number') {
                    ...
                    if (metrics.config.debug) {
                        console.error("Invalid argument passed to mixpanel.people.remove - values must be scalar");
                        console.error("Passed " + key + ':', val);
                    }
                    return;
                }
  ...
  metrics.groups = {
          ...
          */
          set_once: function(group_key, group_id, prop, to, modifiers, callback) {
              ...
              this._set(group_key, group_id, $set, callback, modifiers);
          },
          ...
          */
          ...
          // used internally by set and set_once
          _set: function(group_key, group_id, $set, callback, options) {
              options = options || {};
              var set_key = (options && options.set_once) ? "$set_once" : "$set";
              ...
              if (metrics.config.debug) {
                  console.log("Sending the following data to Mixpanel (Groups):");
                  console.log(data);
              }

              metrics.send_request({ method: "GET", endpoint: "/groups", data: data }, callback);
          },
          ...
          */
          delete_group: function(group_key, group_id, modifiers, callback) {
              ...
              if (metrics.config.debug) {
                  console.log("Deleting group profile -", "group_key:", group_key, "group_id:", group_id);
              }
          ...
          */
          remove: function(group_key, group_id, data, modifiers, callback) {
              ...
              if (typeof(data) !== 'object' || util.isArray(data)) {
                  if (metrics.config.debug) {
                      console.error("Invalid value passed to mixpanel.groups.remove - data must be an object with" +
                        " scalar values");
                  }
                  return;
              }
              ...
              Object.keys(data).forEach(function(key) {
                  ...
                  if (typeof(val) === 'string' || typeof(val) === 'number') {
                      ...
                      if (metrics.config.debug) {
                          console.error("Invalid argument passed to mixpanel.groups.remove - values must be scalar");
                          console.error("Passed " + key + ':', val);
                      }
                      return;
                  }
          ...
          union: function(group_key, group_id, data, modifiers, callback) {
              ...
              if (typeof(data) !== 'object' || util.isArray(data)) {
                  if (metrics.config.debug) {
                      console.error("Invalid value passed to mixpanel.groups.union - data must be an object with" +
                        " scalar" +
                        " or array values");
                  }
                  return;
              ...
              Object.keys(data).forEach(function(key) {
                  ...
                  if (util.isArray(val)) {
                      ...
                      if (metrics.config.debug) {
                          console.error("Invalid argument passed to mixpanel.groups.union - values must be a scalar" +
                            " value" +
                            " or array");
                          console.error("Passed " + key + ':', val);
                      }
          ...
          unset: function(group_key, group_id, prop, modifiers, callback) {
              ...
              if (util.isArray(prop)) {
                  ...
                  if (metrics.config.debug) {
                      console.error("Invalid argument passed to mixpanel.groups.unset - must be a string or array");
                      console.error("Passed: " + prop);
                  }
                  return;
              }
