...
var create_client = function (token, config) {
    ...
    metrics.send_request = function (endpoint, data, callback) {
        ...
        if (endpoint === '/import' || endpoint === '/funnels') {
            var key = metrics.config.key;
            if (!key) {
                throw new Error("The Mixpanel Client needs a Mixpanel api key when importing old events or fetching funnels: `init(token, { key: ... })`");
            }
    ...
    metrics.funnels = function (params, callback) {
        if (typeof(params) === 'function' || !params) {
            callback = params;
            params = {};
        }

        if (params.funnel_id === void 0) {
            throw new Error("The funnels method requires you to specify a funnel ID");
        }

        metrics.track(null, params, callback);
    };
