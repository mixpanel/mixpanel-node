...
var create_client = function(token, config) {
    ...
    */
    ...
    metrics.track = function(event, properties, callback) {
        var mixpanel_track_age_limit = 60 * 60 * 24 * 5, // 5 days in seconds
            endpoint = "/track";
        ...
        if (properties.time) {
            ...
            if (properties.time < Date.now()/1000 - mixpanel_track_age_limit) {
                throw new Error("`track` not allowed for event more than 5 days old; use `import` endpoint");
            }
        }

        metrics.send_event_request(endpoint, event, properties, callback);
    };
