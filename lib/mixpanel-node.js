...
var create_client = function(token, config) {
    ...
    */
    metrics.import = function(event, time, properties, callback) {
        ...
        metrics.track_import(event, properties, '/import', callback);
    };
    ...
    */
    ...
    metrics.people = {
        /**
            people.update(distinct_id, data, callback)
            ---
            update properties on an user record in engage,
            using $set, $set_once, $add, $append, $union, etc
            ...
            usage:
                ...
                data = {
                    $time: time,
                    $ip: ip,
                    $set: { 'gender': 'm', $email: 'bob@bob.bob' },
                    $add: { 'Log In': 1 }
                };
                mixpanel.people.update('bob', data);

        */
        update: function(distinct_id, data, callback) {
        ...
        */
        ...
        /**
            people.union(distinct_id, prop, value, callback)
            ---
            Add a value to a set-valued people analytics property.

            usage:

                // add a value to a set, creating it if needed
                mixpanel.people.union('$ios_devices', apnt);

                // like mixpanel.people.set(), you can add multiple properties at once:
                mixpanel.people.union({
                    set1: 'bob',
                    set2: 123
                });
        */
        union: function(distinct_id, prop, value, callback) {
            var $union = {};
