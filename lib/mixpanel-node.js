...
var create_client = function(token, config) {
    ...
    */
    ...
    metrics.import_batch = function(event_list, options, callback) {
        metrics._batch( '/import', event_list, options, callback );
    };

    metrics._batch = function(endpoint, event_list, options, callback) {
        ...
        var send_next_batch = function() {
            ...
            for (var ei = event_group_idx; ei < total_events && ei < event_group_idx + batch_size; ei++) {
                ...
                if ( endpoint == '/engage' ) {
                    ...
                    e = merge_modifiers(e, options);
                    if (e.hasOwnProperty("$time")) {
                        e.$time = parse_time(modifiers.$time);
                    }
                    if (!e.$token) {
            ...
            if (event_batch.length > 0) {
                metrics.send_request(endpoint, event_batch, function(e) {
    ...
    */
    ...
    metrics.people = {
        ...
        */
        ...
        /**
            people.batch(operations, options, callback)
            ---
        ...
        */
        batch : function(operations, options, callback) {
            metrics._batch( '/engage', operations, options, callback );
        }
    };

    var merge_modifiers = function(data, modifiers) {
        if (modifiers) {
            if (modifiers.$ignore_time && !data.hasOwnProperty('$ignore_time')) {
                data.$ignore_time = modifiers.$ignore_time;
            }
            if (modifiers.hasOwnProperty("$ip") && !data.hasOwnProperty('$ip')) {
                data.$ip = modifiers.$ip;
            }
            if (modifiers.hasOwnProperty("$time") && !data.hasOwnProperty('$time')) {
