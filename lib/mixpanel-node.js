...
var create_client = function(token, config) {
    ...
    */
    ...
    metrics.post_request = function(endpoint, data, callback) {
        var postUrl = metrics.config.protocol + '://';
        postUrl += metrics.config.host + ':';
        postUrl += metrics.config.port || (metrics.config.protocol == 'http' ? 80 : 443);
        postUrl += '/' + endpoint;
        ...
        request(options, function(error, response, body) {
            ...
            if (response.statusCode !== 200) {
                error = new Error('Mixpanel REST HTTP status ' + response.statusCode);
                console.error(error);
                return callback(error);
            }
            try {
                ...
                error = new Error('Mixpanel REST API returned invalid JSON response');
                console.error(error);
                return callback(error);
            }
            if (body.status === 0) {
                error = new Error('Mixpanel REST API reported an error: ' + body.error);
                console.error(error);
                return callback(error);
            }
            callback();
    ...
    /**
        track_import(event, properties, endpoint, callback)
        ---
        common code for track and import.

        event:string                    the event name
        properties:object               additional event properties to send
        endpoint:string                 '/track' or '/import'
        callback:function(err:Error)    callback is called when the request is
                                        finished or an error occurs
    */
    metrics.track_import = function(event, properties, endpoint, callback) {
        if (typeof(properties) === 'function' || !properties) {
            callback = properties;
            properties = {};
        }

        // if event is older than ~5 days, use /import endpoint
        if (typeof(properties.time) === 'number') {
            if ((Date.now()/1000 - properties.time) > (5*24*60*60-60)) {
                endpoint = '/import';
            }
        }
    ...
    */
    metrics.track = function(event, properties, callback) {
        ...
        metrics.track_import(event, properties, '/track', callback);
    };
    ...
    */
    ...
    metrics.people = {
        ...
        */
        ...
        /**
            Performs a similar operation as `mixpanel.people.set()`, except it
            returns a stream-like object, whose `.write(object, callback)` method
            sends the updates to the server (in batches of 50, to optimize HTTP
            traffic). Make sure to call `.end(callback)` when you're done, to flush
            any remaining data.

            Some fields in `object` have special meaning:
                $distinct_id (required),
                $ip (optional, defaults to 0),
                $ignore_time (optional, defaults to false).

            For the meaning of these fields, consult the Mixpanel documentation.
            All the other fields are interpreted as data for the update operation.

            Example usage:

                var stream = mixpanel.people.streamed_set();
                stream.write({
                  $distinct_id: ...,
                  property1: value1,
                  property2: value2,
            ...
            Gets translated into:
                ...
                POST http://api.mixpanel.com/engage
                [
                  {
                    '$token': ...,
                    '$distinct_id' : ...,
            ...
            Where "$set" is the update operation specific to `set()` / `streamed_set()`.
         */
        streamed_set: function() {
            return metrics.people._streamed_update('$set');
        },

        /**
         * Performs a similar function as `mixpanel.people.set_once()`, except
         * using the API of `streamed_set()`.
         */
        streamed_set_once: function() {
            return metrics.people._streamed_update('$set_once');
        },

        /**
         * Performs a similar function as `mixpanel.people.increment()`, except
         * using the API of `streamed_set()`.
         */
        streamed_increment: function() {
            return metrics.people._streamed_update('$add');
        },

        _streamed_update: function(operationName) {
            var batch = [];
            var batchMaxSize = 50;

            function translate(object) {
                if (!object['$distinct_id']) {
                    throw new Error('Required `obj.$distinct_id` is missing in `stream.write(obj)`');
                }
                object = extend({}, object);
                var data = {
                    '$token': token,
                    '$distinct_id': object['$distinct_id'],
                    '$ip': object['$ip'] || 0,
                    '$ignore_time': object['$ignore_time'] || false
                };
                data[operationName] = object;

                delete object['$distinct_id'];
                delete object['$ip'];
                delete object['$ignore_time'];
            ...
            return {
                write: function(object, callback) {
                    var data = translate(object);
                    batch.push(data);
                    if (batch.length < batchMaxSize) {
                        return process.nextTick(callback);
                    }
                    metrics.post_request('engage', batch, function(error) {
                        batch.length = 0;
                        return callback(error);
                    });
                },
