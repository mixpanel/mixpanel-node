...
var create_client = function(token, config) {
    ...
    */
    ...
    metrics.import_batch = function(event_list, callback) {
        var data_length = event_list.length,
            num_requests = Math.ceil(data_length / MP_BATCH_SIZE),
            completed_requests = 0,
            event_group_idx = 0,
            request_errors = [],
            event_batch;

        for (var ri = 0; ri < num_requests; ri++) {
            event_batch = [];
            for (var ei = event_group_idx; ei < data_length && ei < event_group_idx + MP_BATCH_SIZE; ei++) {
                var properties = event_list[ei].properties;
