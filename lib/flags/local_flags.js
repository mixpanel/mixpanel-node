...
const { normalizedHash, lowercaseAllKeysAndValues, lowercaseLeafNodes } = require("./utils");
...
class LocalFeatureFlagsProvider extends FeatureFlagsProvider {
  ...
  _isRuntimeRuleSatisfied(rollout, context) {
    return apply(lowercaseLeafNodes(rollout.runtime_evaluation_rule), this._extractRuntimeParameters(context));
  }
