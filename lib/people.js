const {merge_modifiers} = require('./profile_helpers');

const ENGAGE_ENDPOINT = '/engage';

class MixpanelPeople {
    constructor(mp_instance) {
        this.mp_instance = mp_instance;
    }
    ...
    */
    set_once(distinct_id, prop, to, modifiers, callback) {
        ...
        this.mp_instance._helpers._set(prop, to, modifiers, callback, {identifiers, set_once: true, endpoint: ENGAGE_ENDPOINT});
    }
    ...
    */
    set(distinct_id, prop, to, modifiers, callback) {
        const identifiers = {$distinct_id: distinct_id};
        this.mp_instance._helpers._set(prop, to, modifiers, callback, {identifiers, endpoint: ENGAGE_ENDPOINT});
    }
    ...
    */
    increment(distinct_id, prop, by, modifiers, callback) {
        ...
        if (typeof(prop) === 'object') {
            ...
            for (const [key, val] of Object.entries(prop)) {
                if (isNaN(parseFloat(val))) {
                    if (this.mp_instance.config.debug) {
        ...
        this.mp_instance.send_request({ method: "GET", endpoint: "/engage", data: data }, callback);
    }
    ...
    */
    delete_user(distinct_id, modifiers, callback) {
        const identifiers = {$distinct_id: distinct_id};
        this.mp_instance._helpers._delete_profile({identifiers, modifiers, callback, endpoint: ENGAGE_ENDPOINT});
    }
    ...
    */
    remove(distinct_id, data, modifiers, callback) {
        const identifiers = {'$distinct_id': distinct_id};
        this.mp_instance._helpers._remove({identifiers, data, modifiers, callback, endpoint: ENGAGE_ENDPOINT})
    }
    ...
    */
    union(distinct_id, data, modifiers, callback) {
        const identifiers = {$distinct_id: distinct_id};
        this.mp_instance._helpers._union({identifiers, data, modifiers, callback, endpoint: ENGAGE_ENDPOINT});
    }
    ...
    */
    unset(distinct_id, prop, modifiers, callback) {
        const identifiers = {$distinct_id: distinct_id};
        this.mp_instance._helpers._unset({identifiers, prop, modifiers, callback, endpoint: ENGAGE_ENDPOINT});
    }
};
